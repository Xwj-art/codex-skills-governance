...
description: >
  ...
  execution. In Codex, subagent review is used only when the user explicitly asks for subagents,
...
Create implementation plans with explicit task dependencies optimized for parallel execution. This skill can be ran inside or outside of Plan Mode. In Codex, treat subagent usage as optional and allowed only when the user explicitly asks for subagents, delegation, or parallel agent work.
...
6. **Review Before Yield**: Review the plan for gaps before finalizing; use a subagent only when explicitly authorized by the user.
...
After saving, review the plan for:
...
If the user explicitly authorized subagents/delegation, spawn a subagent for
this review. Otherwise perform the review locally. If the review finds
actionable feedback, revise the plan before yielding.
